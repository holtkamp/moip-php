...
class MoIP
{
    ...
    public function setPagamentoDireto($params)
    {
        ...
        )
        {
            throw new InvalidArgumentException("Voc deve especificar uma institui‹o de pagamento v‡lida quando".
                " a forma de forma de pagamento Ž via dŽbito ou cartao");
        }
    ...
    public function addComission($param)
    {
        ...
        {
            //TODO: 20120326, MH: it seems that this element has to be named 'Comissionamento' instead of 'Comissao'
    ...
    public function addDelivery($params)
    {
        ...
        if ($params['tipo']=='correios')
        {
            ...
            {
                throw new InvalidArgumentException('ƒ necess‡rio especificar os '.
                    'par‰metros dos correios quando o '.
                    'tipo de frete Ž Correios');

            }

            if (!isset($params['correios']['peso']) or !isset($params['correios']['forma_entrega']))
            {
                throw new InvalidArgumentException('ƒ necess‡rio passar os par‰metros'.
        ...
        else
        {
            if (!isset($params['valor_fixo']) and !isset($params['valor_percentual']))
                throw new InvalidArgumentException('Voc deve especificar valor_fixo ou valor_percentual quando o tipo de frete Ž pr—prio');
        }
    ...
    public function verifyPagamentoDireto($login_moip,$client=null)
    {
        if (!isset($this->credential))
            throw new Exception("Voc deve especificar as credenciais (token/key) da API antes de chamar este mŽtodo");
